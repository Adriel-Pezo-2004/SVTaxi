...
const EditarIngresoPage = () => {
  ...
  useEffect(() => {
    if (!id) return
    setFetching(true)
    fetch(`/api/ingresos/${id}`, { credentials: "include" })
      .then(async (res) => {
        if (!res.ok) {
        ...
        setIngresoTotal(data.IngresoTotal?.toString() || "")
        setGastoGaso(data.gastoGaso?.toString() || "")
        setGastoComision(data.gastoComision?.toString() || "")
        setFetching(false)
      })
      .catch(() => {
        setError("Error de red al cargar ingreso.")
        setFetching(false)
      })
  }, [id])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setIngresoNeto(
          Math.round(
            (Number(IngresoTotal) - Number(gastoGaso) - Number(gastoComision)) * 100
          ) / 100
        )
        setTimeout(() => router.push("/home"), 1500)
      } else {
        setError(data.error || "Error al actualizar. Intenta de nuevo.")
      }
    } catch {
  ...
  const calculatePreview = () => {
    if (IngresoTotal && gastoGaso && gastoComision) {
      ...
      return Math.round((total - gaso - comision) * 100) / 100
    }
    return null
  }
    ...
    <div className="container py-10 max-w-md mx-auto px-4 sm:px-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div className="flex items-center gap-2">
          ...
          {id && (
            <span className="ml-3 px-2 py-1 rounded bg-muted text-xs text-muted-foreground border border-muted-foreground/20">
              ID: {id}
            </span>
          )}
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Edita los detalles de tu jornada de trabajo</CardDescription>
        </CardHeader>
        <CardContent>
          {fetching ? (
                  ...
                  <AlertDescription className="flex flex-col">
                    ...
                    {ingresoNeto !== null && (
                      <span className="mt-2 font-semibold text-primary">
                        Ingreso Neto: S/{ingresoNeto.toFixed(2)}
                      </span>
                    )}
                  </AlertDescription>
                </Alert>
              ...
              {previewNeto !== null && (
                <div className="mt-4 p-3 bg-accent rounded-md">
                  ...
                  <p className="text-lg font-semibold text-primary">
                    Ingreso Neto: S/{previewNeto.toFixed(2)}
                  </p>
                </div>
              )}
